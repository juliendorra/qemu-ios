...
| T7 | **N45AP: slide-to-unlock dies after the FIRST sleep/wake cycle** (user-reported, REPRODUCED by `scripts/lock-unlock-probe.py --board n45ap --cycles 3`: cycle 1 `unlocked`, cycles 2+ `stuck-locked`). Not the readiness gate — the touch IS delivered (`[TOUCH] mouse DOWN`). The difference is in the driver's answer to the ATN: cycle 1 reads frames with `0xEB` (READ_INTERRUPT_DATA); after the next wake it sends `0xEC` once, then `0xE4`/`0xE1`, and never reads the frame. `0xEC` was unimplemented (garbage reply); it is now answered like `0xEB`, which did NOT change the outcome — so something else in the post-wake controller state is wrong. Next: diff the full Z2 dialogue of cycle 1 vs cycle 2 from the ATN backwards (`IT_MT_TRACE=1`), and check whether the ATN group/bit or the driver's interrupt unmask survives the AP reset. | Touch is unusable after the first lock on the iPod. | **TODO** |
