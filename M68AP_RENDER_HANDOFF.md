...
| T7 | **N45AP: slide-to-unlock dies after a sleep/wake cycle — STILL OPEN.** The `R_RXCNT` transaction framing passed the headless probe 4/4 but made the packaged APP *worse* (even the first unlock stopped responding, where it used to work), so it is now behind `IT_SPI_FRAMING=1` and OFF by default. The desync it targets is real and measured; `RXCNT`→0 is simply not equivalent to "the driver abandoned this command". | Touch is unusable after the first lock on the iPod. | **OPEN** |
| T8 | **Fix the test harness before re-attempting T7.** `lock-unlock-probe.py` runs headless, so `gfx_update` never runs and the readiness logic under test is skipped — which I compensated for by CHANGING the model, i.e. validating a configuration nobody runs (the app runs `lcd_refresh` at 60 Hz *and* the timer). It also drives a fast synthetic slide (12 steps / 0.7 s) where a human slide is slower with far more motion frames, and judges success from pixels alone. Needed: (a) a display client (`-vnc` + an RFB client requesting continuous updates) so `gfx_update` runs as in the app; (b) an assertion at the model boundary — count frame reads (`0xEA`/`0xEB` returning data) per touch, so "delivered but ignored" fails even when the screen looks right; (c) a configurable, human-paced gesture swept across timings. | A harness that disagrees with the product is worse than none: this one certified a change that regressed the app. | **TODO — blocks T7** |
