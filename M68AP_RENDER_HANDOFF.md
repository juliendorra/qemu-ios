...
| T6 | **Make the generated NAND accept guest writes.** `/private/var` is mounted read-WRITE, yet a booted guest lands ZERO pages in the NAND model, so nothing it creates survives a read-back. Any daemon that must create state spins forever — com.apple.AddressBook retries ~250×/s and pegs the CPU at 98% (iPod: 11–15%), which is why the packaged image currently ships without that daemon. The emulator side is ready (`IT_NAND_WRITABLE=1` makes written pages shadow the pack); the missing part is FTL/VFL write support on the constructed tree. | Restores Contacts, lets settings persist, and removes a whole class of retry loops. | **TODO** |
