...
| T2 | **Model MBX 2D** so `LK_ENABLE_MBX2D=0` is no longer needed. LayerKit otherwise tight-polls `c03b9698` (register-read accessor in `com.apple.driver.AppleMBX`). | Today we force software compositing via a guest plist edit — a shortcut, and the iPod ships the same one. | **PARKED 2026-07-31** — the §4 gate measured: compositing is 2–4% of guest CPU natively (fidelity-only), and today ~2–4% of the wasm vCPU thread too; the bound rises as the wasm speed campaign removes engine overhead, so re-run the arithmetic then. **UNBLOCKED 2026-08-01**: the MBX has an MMU (8-entry page directory at regs 0x1000-0x101c) and the model can translate/forward through it (`IT_MBX_MMU=1`); blocked now only on the guest emitting a 2D stream, which it currently declines. Measurement tooling, the wasm caveat and both routes forward in [`MBX_SDO_MMU_HANDOFF.md`](MBX_SDO_MMU_HANDOFF.md). |
