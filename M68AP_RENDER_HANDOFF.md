...
|---|---|---|---|
| T9 | **iPhone OS 1.0: HOME/POWER do nothing while an app is frontmost.** The FIRST in-app press is delivered and handled completely -- both SpringBoard handlers hit for real, `-[SBUIController clickedMenuButton]` runs the display-stack unwind and returns, and the stack unwinds cleanly into the run loop -- and then **SpringBoard SPINS at 0.97 host cores forever**, starving every other process -- which is why GSEvent delivery dies for EVERY process, taps included, and why the LCD and touch stop with it. Ten presses in-app deliver 1 DOWN/1 UP against 10/10 from the home screen and 10/10 on 1.1.4. The wedge is ASYNCHRONOUS (seconds), so no stepper can reach it. Ruled out by measurement: the pin, the IRQ, INTEN, the interrupt ACK, `AppleM68Buttons`, event ROUTING and the port (`_ResetEventPortSet`/`_GSRegisterApplicationPort` never fire on 1.0), both SpringBoard handlers, and `clickedMenuButton`. **ROOT CAUSE FOUND 2026-07-29: the spin is in `com.apple.driver.AppleMBX`, `do { v = mbx_read(base, 0x12C); } while (!(v & 0x40));` -- a completion bit our do-nothing MBX stub never sets.** So this is the MBX gap (**T1**), not an iPhone OS 1.0 software bug. **PARTIALLY FIXED**: `0x12C` now returns `0x140`, which removes the spin (0.99 -> 0.10 host cores, A/B via `IT_MBX_READY=0`) and restores event delivery (5 DOWN/6 UP over six presses, taps reaching the app). ~~The app still does not visually dismiss~~ **RESOLVED 2026-07-31: `3_home_returns` PASSES on 1.0** (the missing register was a WRITE to 0x12C -- a soft event the model now reflects; the MBX event line is also plumbed, `IT_MBX_IRQ`). Residual: the ~33 s dismissal latency (FinishSurface timeouts; the honest fix is memory-side, see the moratorium at the end of the investigation doc). Full record, including retracted conclusions and the silent harness traps, in [`IN_APP_BUTTON_INVESTIGATION.md`](IN_APP_BUTTON_INVESTIGATION.md). | The iPhone OS 1.0 bundle is usable again past the first app launch; dismissals are slow (~33 s). | **FIXED (latency residual open)** |
| T1 | **Model MBX swap completion / the TVOut SDO IRQ** so the guest driver clears the swap-device field itself, and delete the workaround window entirely. | Removes the last address-dependent hack in the display path, on both boards. | **DONE 2026-07-31** (commits f6a2ee2835→a05a58b458). The wait was never on an MBX register: the missing signal was the **SDO field interrupt**, now modelled in `hw/arm/ipod_touch_tvout.c` (default on; `IT_TVOUT_SDO=0` is the A/B back to stub+window). The zero-window is no longer armed by default; the guest completes `[swapdev+0x160]` itself. Verified: 4A102 boot+lock/unlock, N45AP 3/3 cycles, 1A543a inert. Remaining: verify 3A109a/1C28 once their NANDs are regenerated. Full record in [`MBX_SDO_MMU_HANDOFF.md`](MBX_SDO_MMU_HANDOFF.md). |
| T2 | **Model the MBX legacy command stream, surface-ring retirement, and completion ordering.** | Removes the kernel spin/timeouts without pretending a register acknowledgment drew pixels. | **ACTIVE 2026-08-01.** Forced hardware mode proved the 8-entry MMU, `0x6d8` kick, and 29/29 aperture-to-DRAM writes, but disproved direct clearing of `state1+0x60` and IRQ-first completion. Event `0x10` may be raised only after every `record+0x34c` surface-list node is CPU-traversable; otherwise the real ISR faults. Keep the staged software-renderer fallback until forced MBX2D passes the strict eight-second boot and in-app POWER/wake oracle. See [`MBX_SDO_MMU_HANDOFF.md`](MBX_SDO_MMU_HANDOFF.md). |
...
> **MBX architecture clarification:** all working product paths currently use
> LayerKit's original guest-side software compositor, running as ARM code under
> QEMU. N45AP's supplied NAND and the prepared 4A102 NAND already steer
> LayerKit with `LK_ENABLE_MBX2D=0`; only 1A543a additionally needs the guarded,
> per-launch `_mbx2DInitialize` failure because its older backing-store client
> still submits MBX2D work. The current MBX model does not pass commands to the
> host GPU and does not rasterize forced legacy commands yet; it emulates the
> MMU, shared aperture, transport and part of retirement/completion. The
> per-build routing table and detailed boundary are documented in
