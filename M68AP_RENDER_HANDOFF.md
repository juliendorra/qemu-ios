...
| T7 | **N45AP: slide-to-unlock dies after the FIRST sleep/wake cycle** (user-reported, REPRODUCED by `scripts/lock-unlock-probe.py --board n45ap --cycles 3`: cycle 1 `unlocked`, cycles 2+ `stuck-locked`). Not the readiness gate — the touch IS delivered (`[TOUCH] mouse DOWN`). The difference is in the driver's answer to the ATN: cycle 1 reads frames with `0xEB` (READ_INTERRUPT_DATA); after the next wake it sends `0xEC` once, then `0xE4`/`0xE1`, and never reads the frame. **Byte-level evidence (`IT_MT_TRACE=2`) shows a DESYNC, not a strange command.** In the failing cycle the model is already mid-transaction when the driver's next command byte arrives (`cmd 0xeb 2/16`), so that byte is consumed as *data* and everything after it is misread as new commands — which is why "0xEC/0xED/0xE4" appeared at all. Root cause class: this model has **no transaction framing**. `cur_cmd`/`buf_ind` reset only when the guest clocks exactly `buf_size` bytes, so any transaction the driver ends early (e.g. a 0xEB frame poll with nothing queued) desyncs it permanently. Two fixes were tried and BOTH FALSIFIED: answering `0xEC` like `0xEB` (no change), and framing on chip-select — dead code, because the guest never drives CS through `R_PIN` (**0 CS edges measured** over a full boot with `IT_SPI_CS_TRACE=1`). Next candidate framing signal: the SPI controller's own transfer boundary (`R_CTRL_RUN` going high with a fresh TX FIFO, or the FIFO draining) in `hw/arm/ipod_touch_spi.c:apple_spi_run`. | Touch is unusable after the first lock on the iPod. | **TODO** |
